...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        if (providerId == null) {
            ...
            throw new OAuth2AuthenticationException(new OAuth2Error("invalid_user"), "No provider id (sub) present");
        }
        ...
        try {
            ...
            if (opt.isPresent()) {
                ...
                if (email != null) {
                    ...
                    if (byEmail.isPresent()) {
                        user = byEmail.get();
                        log.info("Found existing user by email={}, linking with provider {}", email, registrationId);
            ...
            log.info("OAuth2 login completed successfully for user id={}, providerId={}", user.getId(), providerId);

            return new DefaultOAuth2User(authorities, mapped, "sub");

        } catch (Exception e) {
            log.error("Error during OAuth2 user processing", e);
            throw new OAuth2AuthenticationException(new OAuth2Error("user_creation_failed"),
